...
module.exports.run = async (bot, message, args) => {
    ...
    if (wUser.id == message.author.id) return message.channel.send("You can't warn yourself").then(msg => {msg.delete(5000)});
    let wReason = args.join(" ").slice(22);
    if (!wReason) return message.channel.send("Please specify a warn reason, for example 'spamming' ").then(msg => {msg.delete(5000)});
